...
job("$context.jobFullName-bluetooth") {
    description("Make a release of trikot.bluetooth")
    logRotator(5)
    scm {
        git {
            branch("${GIT_BRANCH}")
...
job("$context.jobFullName-bluetooth-android-ktx") {
    description("Make a release of trikot.bluetooth.android-ktx")
    logRotator(5)
    scm {
        git {
            branch("${GIT_BRANCH}")
